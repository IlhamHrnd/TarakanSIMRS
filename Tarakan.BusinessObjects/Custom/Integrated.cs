...
namespace Tarakan.BusinessObjects.Custom
{
    public class Integrated
    {
        ...
        {
            return $"<a href=\"#\" onclick=\"javascript:onDeleteIntegratedNote('{rimId}', '{isFromAskep}'); return false;\" alt=\"Void\" title=\"Void\"><i class=\"{BaseIcon.Trash}\"></i></a>";
        }

        public static string IntegratedNoteUnDelete(string rimId, bool isFromAskep)
        {
            return $"<a href=\"#\" onclick=\"javascript:onUnDeleteIntegratedNote('{rimId}', '{isFromAskep}'); return false;\" alt=\"Unvoid\" title=\"Unvoid\"><i class=\"{BaseIcon.Retweet}\"></i></a>";
        }
        ...
        public static string IntegratedNotePrint(bool isDeleted, string srMedicalType, bool isFromAskep, string rimId)
        {
            ...
            return Const.MedicalNotes2.Contains(srMedicalType) && isFromAskep == false && isDeleted == false
                ? $"<a href=\"#\" onclick=\"javascript:printPreviewIntegratedNotes('{rimId}'); return false;\" alt=\"Print Integrated Notes\" title=\"Print Integrated Notes\"><i class=\"{BaseIcon.Print}\"></i></a><br /><br />"
                : string.Empty;
        }
        ...
        public static string IntegratedNoteVerif(RegistrationInfoMedicDto rim, bool isUserParamedicDpjp, string parId, string srUserType)
        {
            if (IsAccessVerif(rim, parId, srUserType, isUserParamedicDpjp))
                return $"<a style=\"cursor:pointer;\" onclick=\"javascript:openCpptVerification('{rim.RegistrationInfoMedicId}_{rim.IsFromAskep}','{rim.ParamedicId}','{isUserParamedicDpjp}')\" alt=\"Verification Integrated Notes\" title=\"Verification Integrated Notes\"><i class=\"{BaseIcon.Checkcircle}\"></i></a><br /><br />";
            else
                return string.Empty;
        }
        ...
        public static string IntegratedEntryAssessment(RegistrationInfoMedicDto rim, string userId)
        {
            ...
            string icon = IsEditable(rim.IsApproved ?? false, rim.CreatedByUserId, userId) ? $"<i class=\"{BaseIcon.Pensquare}\"></i>" : $"<i class=\"{BaseIcon.Folder}\"></i>";
